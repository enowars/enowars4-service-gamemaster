...
namespace GamemasterChecker
{
    public class GamemasterChecker : IChecker
    {
        ...
        private async Task<CheckerResultMessage> HavokChat(CheckerTaskMessage task, CancellationToken token)
        {
            ...
            using GamemasterSignalR src1 = new GamemasterSignalR(task.Address, user1, Logger, null, client1, token);
            using GamemasterSignalR src2 = new GamemasterSignalR(task.Address, user2, Logger, tcs, client2, token);
